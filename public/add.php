...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	$db     = $db->getConnection();
	$result = $db->prepare( "INSERT INTO products (`name`,`type`,`colour`,`size`,`description`,`price`) VALUES (:name, :type, 'x', 'x', :description, :price )" );
	$result->bindParam( ':name', $_POST['name'] );
	$result->bindParam( ':type', $_POST['type'] );
	$result->bindParam( ':colour', $_POST['colour'] );
