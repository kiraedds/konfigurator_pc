...
<div id="plytyglowne">
    ...
    <div class="table">
        ...
        $minimum = $db->query('SELECT min(p.price) FROM products p where type="plyty_glowne"');
        $minimum->execute();
        $min = $minimum->fetch(PDO::FETCH_ASSOC);

        $maksimum = $db->query('SELECT max(p.price) FROM products p where type="plyty_glowne"');
        $maksimum->execute();
        $max = $maksimum->fetch(PDO::FETCH_ASSOC);

        if(!isset($_SESSION['role']) || empty($_SESSION['role'])) {
            echo "<form name='filt' action='index.php?value=plytyglowne' method='post'>";
        }
        else if($_SESSION['role'] == 'customer' && $_SESSION['zalogowany'] == 'true') {
            echo "<form name='filt' action='customer.php?value=plytyglowne' method='post'>";
        }
        else if( $_SESSION['role'] == 'seller' ) {
            echo "<form name='filt' action='seller.php?value=plytyglowne' method='post'>";
        }
        ...
        if($_SERVER["REQUEST_METHOD"]=="POST"){
            ...
            $result = $db->query('SELECT p.id, p.name, p.description, p.price, i.src FROM products p, images i where p.id=i.product_id and type="plyty_glowne" and p.price >= "'.$min1.'" and p.price <= "'.$max1.'"');
            $result->execute();
            ...
            $result = $db->query('SELECT p.id, p.name, p.description, p.price, i.src FROM products p, images i where p.id=i.product_id and type="plyty_glowne" ');
            $result->execute();
